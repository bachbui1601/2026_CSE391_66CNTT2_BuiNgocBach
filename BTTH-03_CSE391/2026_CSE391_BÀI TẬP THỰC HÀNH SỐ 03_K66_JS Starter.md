...
Điều này cho thấy khi trang web được tải, trình duyệt đã đọc và thực thi file main.js, và lệnh console.log() đã in ra thông báo trong Console.
---
...
>   - `boolean` là: Kiểu dữ liệu chỉ có hai giá trị là true hoặc false, dùng để biểu diễn đúng/sai.
...
Xin chào, mình là Bach, năm nay mình 20 tuổi.
> - Nếu em quên dấu `;` hoặc quên dấu `+`, điều gì xảy ra? Trình duyệt báo lỗi thế nào?
trình duyệt sẽ báo lỗi trong Console, Uncaught SyntaxError, Unexpected identifier
---

#### Bước 5: Phản tư nhanh (Reflection)

> - Điều thú vị nhất em vừa khám phá được về console là gì?
Console cho phép chạy thử JavaScript trực tiếp mà không cần sửa file, giúp kiểm tra code rất nhanh
> - Em gặp lỗi cú pháp nào? Em đã xử lý bằng cách nào (tự sửa, hỏi bạn, đọc lỗi, tìm Google, …)?Thiếu dấu +, Viết sai tên biến, Thiếu dấu ngoặc, Đọc lỗi trong Console, Sửa lại code
---
...
hiểu đơn giản thì là nếu xét điều kiện đàu tiên mà sai thì sẽ xét xuống các điều kiện ở phía dưới
...
Tái sử dụng code,Code gọn gàng hơn
---
...
> Câu hỏi:
> - `document.getElementById("status")` đang làm gì?Lấy phần tử HTML có id="status" để JavaScript có thể thao tác.
> - Sự kiện `"click"` xảy ra khi nào?Khi người dùng nhấn chuột vào phần tử (button).
> - Trong đoạn code trên, khi nhấn nút `btnHello`, điều gì thay đổi trên trang?Nội dung của đoạn <p id="status"> được thay đổi thành:
...
xảy ra khi nhấn chuột
xảy ra khi người dùng gõ hoặc thay đổi nội dung ô input
...
>   DOM là mô hình biểu diễn trang HTML dưới dạng cây các phần tử, cho phép JavaScript truy cập và thay đổi nội dung, cấu trúc và style của trang web.
...
> Câu hỏi:
> - Điểm giống nhau về chức năng giữa 2 đoạn code trên là gì?Cả hai đều dùng để bắt sự kiện click và thực hiện hành động.
> - Điểm khác nhau về cú pháp là gì (`document.getElementById` vs `$("#id")`, `addEventListener` vs `.on`)?document.getElementById()
$("#id")
addEventListener()
.on()
> - Em hãy tra cứu nhanh “What is jQuery used for?” và ghi 2 ý chính:
>   1. Thao tác DOM dễ dàng hơn
>   2. Xử lý sự kiện và hiệu ứng nhanh hơn với cú pháp ngắn gọn.
